...
export default function ArticlePage() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setProgress(
        total > 0
          ? Math.min(100, (h.scrollTop / total) * 100)
          : 0
      );
    };
   ...
   * Table of contents
   *
   * Only Markdown level-2 headings (##) are included.
   *
   * Markdown structure:
   * #  = document/article-level heading
   * ## = sections shown in the TOC
   * ### = subsections, not shown in the TOC
   */
  ...
  return (
    <PageLayout>
      <Helmet>
        <title>
          {article.title} - ObservabiliTrends
        </title>
